...
describe("content", () => {
  ...
  it("upcomingEvents has 6 entries each category in allowed set and title/summary non-empty", () => {
    ...
    // href is optional; when present it must be a non-empty absolute URL
    for (const event of upcomingEvents) {
  ...
  it("priests has 4 OFM friars each name/role non-empty", () => {
    expect(priests).toHaveLength(4);
  ...
  it("ppcMembers has 6 entries each role/name non-empty", () => {
