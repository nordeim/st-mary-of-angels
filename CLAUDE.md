...
| `pnpm test` / `npm run test` | Vitest `jsdom` — `vitest run` | ✅ | **17 files / 109 tests** — `utils/cn` 5 + `data/nav` 7 + `data/content` 10 + `data/site` 7 + `ui/Button` 9 + `SkipLink` 3 + `ui/Accordion` 6 + `SafeImage` 6 + `Header` 16 + `BackToTop` 7 + `pages/Ministries` 3 + `pages/cta-bands` 3 + `Layout` 2 + `hooks/useScrollProgress` 4 + `ScrollProgress` 2 + `head` 13 + `security-headers` 6 via `src/test/setup.ts` |
...
| `pnpm test:e2e` / `npm run test:e2e` | Playwright `chromium` — `playwright test` (36 tests: smoke 11 + navigation 8 + ministries 4 + give-faq 4 + enhancements 9 in `e2e/`) | ✅ | Green — Bukit Batok routes (`/worship`/`/ministries`/`/serve`/`/give` + `#mass`/`#liturgical` etc.) + drawer same-route close regression + rise-in entrance + event chips + back-to-top + aria-current nav + Round-2 audit (CTA-band cream headings, head completeness, page-in transitions, scroll-progress rail + ring, drawer aria-current) + Round-4 (modal drawer dialog/aria-modal/trapped focus) |
...
pnpm test               # vitest jsdom — 17 files / 109 tests
pnpm test:e2e           # playwright chromium — 36 tests (5 specs)
...
e2e/ (5 specs / 36 tests) # smoke (11) + navigation (8) + ministries (4) + give-faq (4) + enhancements (9) + helpers.ts
...
- Tests: `*.test.{ts,tsx}` adjacent to source — **17 files / 109 tests**: `src/utils/cn.test.ts` (5), `src/data/nav.test.ts` (7), `src/data/content.test.ts` (10), `src/data/site.test.ts` (7), `src/components/ui/Button.test.tsx` (9), `src/components/SkipLink.test.tsx` (3), `src/components/ui/Accordion.test.tsx` (6), `src/components/SafeImage.test.tsx` (6), `src/components/Header.test.tsx` (16), `src/components/BackToTop.test.tsx` (7), `src/components/Layout.test.tsx` (2), `src/components/ScrollProgress.test.tsx` (2), `src/hooks/useScrollProgress.test.ts` (4), `src/pages/Ministries.test.tsx` (3), `src/pages/cta-bands.test.tsx` (3), `src/head.test.ts` (13), `src/security-headers.test.ts` (6) + `src/test/setup.ts` (jest-dom + IntersectionObserver mock + scrollTo/scrollIntoView + matchMedia stubs). `vite.config.ts` `test.exclude` keeps `e2e/**` out of unit runs; `e2e/*.spec.ts` is Playwright only.
...
- `pnpm lint`, `pnpm typecheck`, `pnpm test` (17 files / 109 tests), `pnpm test:e2e` (36 tests, chromium), and `pnpm build` are all green (145 total — 17 unit files + 5 E2E specs via `lint && typecheck && test && test:e2e && build`).
...
| 5 | Testing Strategy (wired — 17 unit files / 109 tests + 36 E2E, St Mary data) | Yes | ✅ |
