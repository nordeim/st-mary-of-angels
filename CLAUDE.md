...
| `pnpm test` / `npm run test` | Vitest `jsdom` — `vitest run` | ✅ | **17 files / 104 tests** — `utils/cn` 5 + `data/nav` 7 + `data/content` 10 + `data/site` 7 + `ui/Button` 9 + `SkipLink` 3 + `ui/Accordion` 6 + `SafeImage` 6 + `Header` 11 + `BackToTop` 7 + `pages/Ministries` 3 + `pages/cta-bands` 3 + `Layout` 2 + `hooks/useScrollProgress` 4 + `ScrollProgress` 2 + `head` 13 + `security-headers` 6 via `src/test/setup.ts` |
| `pnpm test:watch` | Vitest watch mode (`vitest`) | ✅ | Watches 17 files |
...
pnpm test               # vitest jsdom — 17 files / 104 tests
...
- Tests: `*.test.{ts,tsx}` adjacent to source — **17 files / 104 tests**: `src/utils/cn.test.ts` (5), `src/data/nav.test.ts` (7), `src/data/content.test.ts` (10), `src/data/site.test.ts` (7), `src/components/ui/Button.test.tsx` (9), `src/components/SkipLink.test.tsx` (3), `src/components/ui/Accordion.test.tsx` (6), `src/components/SafeImage.test.tsx` (6), `src/components/Header.test.tsx` (11), `src/components/BackToTop.test.tsx` (7), `src/components/Layout.test.tsx` (2), `src/components/ScrollProgress.test.tsx` (2), `src/hooks/useScrollProgress.test.ts` (4), `src/pages/Ministries.test.tsx` (3), `src/pages/cta-bands.test.tsx` (3), `src/head.test.ts` (13), `src/security-headers.test.ts` (6) + `src/test/setup.ts` (jest-dom + IntersectionObserver mock + scrollTo/scrollIntoView + matchMedia stubs). `vite.config.ts` `test.exclude` keeps `e2e/**` out of unit runs; `e2e/*.spec.ts` is Playwright only.
...
- `pnpm lint`, `pnpm typecheck`, `pnpm test` (17 files / 104 tests), `pnpm test:e2e` (35 tests, chromium), and `pnpm build` are all green (139 total — 17 unit files + 5 E2E specs via `lint && typecheck && test && test:e2e && build`).
...
| 5 | Testing Strategy (wired — 17 unit files / 104 tests + 35 E2E, St Mary data) | Yes | ✅ |
