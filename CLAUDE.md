...
src.orig/                # Archived previous port — St Joseph's Church (Bukit Timah) — retained locally as src.orig/ (35 src + 16 tests + 1 setup), ignored via .gitignore (not committed); NOT linted/built; eslint + vite-watch ignore entries are active guards; lineage: Rother Shrine → St Joseph BT (src.orig) → St Mary of the Angels (src)
```
